...
function makeTwoTurnsMoveTile() {
    let tile = document.createElement("span");
    tile.className = "movement-tile-2turn awbwenhancements-range-tile";
    return tile;
}
...
class MoveRangePreview {
    constructor(gamemap, terrainInfo, players) {
        ...
        this.enableTwoMoveHighlight = true;
    ...
    showMoveRangeFor(unit, selected) {
        ...
        this.selectedUnit = selected ? unit : null;
        let positions1 = this.getPositionsInMoveRange(unit);

        if (this.enableTwoMoveHighlight) {
            let positions2 = this.getPositionsInTwoTurnsRange(unit, positions1);
            let positions2Only = {};
            for (let id in positions2) {
                if (!(id in positions1)) {
                    positions2Only[id] = positions2[id];
                }
            }
            this.displayTwoTurnsRangeTiles(positions2Only, positions2);
        }

        this.displayMoveRangeTiles(positions1);
    }
    ...
    getPositionsInTwoTurnsRange(unit, positions1) {
        let unitData = kUnitsByName[unit.unit];
        let playerInfo = this.playersByCountry[unit.country.code];
        let moveCostMatrix = lookupMoveCostMatrix(this.weather, playerInfo);
        let startingMove = lookupUnitMoveForCo(unitData, playerInfo);
        ...
        let positions2 = {};
        for (let posId in positions1) {
            let startCoord = positions1[posId];

            let queue = [{coord: startCoord, remainingMove: startingMove}];
            let visited = {};
            visited[posId] = startingMove;
            positions2[posId] = startCoord;
            ...
            while (queue.length > 0) {
                for (let {coord, remainingMove} of queue) {
                    ...
                    for (let neighborId in neighbors) {
                        ...
                        let newRemainingMove = remainingMove - terrainCost;
                        positions2[neighborId] = neighbor;
                        nextQueue.push({coord: neighbor, remainingMove: newRemainingMove});
                    }
                }
                queue = nextQueue;
    ...
    displayTwoTurnsRangeTiles(positions, allPositions) {
        this.displayRangeTiles(positions, makeTwoTurnsMoveTile, allPositions);
    }
    ...
    displayRangeTiles(positions, tileFactory, allPositions) {
    ...
    refresh() {
        if (this.selectedUnit) {
            ...
            this.clearMoveRange();
            this.showMoveRangeFor(unit, true);
        }
    }
}
