...
class PlayerPanel {
  ...
  updateHtml() {
    ...
    let powerBars = this.panel.getElementsByClassName("power-bar");
    if (powerBars.length !== 0) {
      powerBars[0].title = "Click to manually edit CO power charge";
      powerBars[0].style.cursor = "pointer";
      powerBars[0].addEventListener("click", () => {
        this.openChargeEditModal();
      });
    }
  ...
  openChargeEditModal() {
    let existingModal = document.getElementById("awbw-charge-edit-modal");
    if (existingModal) {
      existingModal.remove();
    }

    let co = kCosByName[this.playerInfo.co_name] || { co_max_power: 270000, co_max_spower: 540000 };
    let maxCharge = this.liveInfo.players_co_max_spower || co.co_max_spower || 540000;
    let currentCharge = Math.min(maxCharge, Math.max(0, this.liveInfo.players_co_power || 0));

    // Internal charge = Funds * 10 (9,000 funds = 1 Star = 90,000 charge)
    let maxFunds = Math.round(maxCharge / 10);
    let currentFunds = Math.round(currentCharge / 10);
    let copFunds = Math.round((this.liveInfo.players_co_max_power || co.co_max_power) / 10);

    let username = this.playerInfo.users_username || this.playerInfo.co_name || "Player";

    let modalOverlay = document.createElement("div");
    modalOverlay.id = "awbw-charge-edit-modal";
    modalOverlay.className = "awbwenhancements-charge-modal-overlay";
    modalOverlay.innerHTML = `
      <div class="awbwenhancements-charge-modal-content">
        <div class="awbwenhancements-charge-modal-header">
          <h3>Edit CO Power Charge</h3>
          <span class="awbwenhancements-charge-modal-close">&times;</span>
        </div>
        <div class="awbwenhancements-charge-modal-body">
          <p style="margin-bottom: 10px; font-size: 13px; color: #ddd;">
            Player: <strong>${username}</strong> (${this.playerInfo.co_name})
          </p>
          <label style="display: block; margin-bottom: 6px; font-weight: bold; font-size: 12px; color: #bbb;">
            Charge in Funds (0 to ${maxFunds.toLocaleString()} G):
          </label>
          <input type="number" id="awbw-charge-input-val" class="awbwenhancements-charge-input"
                 value="${currentFunds}" min="0" max="${maxFunds}" step="1000">
          <div id="awbw-charge-stars-hint" style="margin-top: 6px; font-size: 12px; color: #39ff14; font-weight: bold;">
            = ${(currentFunds / 9000).toFixed(2)} Stars
          </div>

          <div style="display: flex; gap: 6px; margin-top: 14px; margin-bottom: 8px;">
            <button type="button" class="awbwenhancements-preset-btn" data-funds="0">Clear (0 G)</button>
            <button type="button" class="awbwenhancements-preset-btn" data-funds="${copFunds}">Full COP (${(copFunds / 9000).toFixed(0)}★)</button>
            <button type="button" class="awbwenhancements-preset-btn" data-funds="${maxFunds}">Full SCOP (${(maxFunds / 9000).toFixed(0)}★)</button>
          </div>
        </div>
        <div class="awbwenhancements-charge-modal-footer">
          <button type="button" id="awbw-charge-cancel-btn" class="awbwenhancements-charge-btn awbwenhancements-btn-cancel">Cancel</button>
          <button type="button" id="awbw-charge-ok-btn" class="awbwenhancements-charge-btn awbwenhancements-btn-ok">OK</button>
        </div>
      </div>
    `;
    ...
    let inputEl = modalOverlay.querySelector("#awbw-charge-input-val");
    let hintEl = modalOverlay.querySelector("#awbw-charge-stars-hint");
    let okBtn = modalOverlay.querySelector("#awbw-charge-ok-btn");
    let cancelBtn = modalOverlay.querySelector("#awbw-charge-cancel-btn");
    let closeBtn = modalOverlay.querySelector(".awbwenhancements-charge-modal-close");
    ...
    const updateHint = () => {
      ...
      let stars = (val / 9000).toFixed(2);
      hintEl.textContent = `= ${stars} Stars (${val.toLocaleString()} / ${maxFunds.toLocaleString()} G)`;
    };

    inputEl.addEventListener("input", updateHint);

    let presetBtns = modalOverlay.querySelectorAll(".awbwenhancements-preset-btn");
    presetBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        let val = parseInt(btn.getAttribute("data-funds")) || 0;
        inputEl.value = val;
        updateHint();
      });
    });
    ...
    const confirmCharge = () => {
      let enteredFunds = parseInt(inputEl.value);
      if (isNaN(enteredFunds)) enteredFunds = 0;
      let internalCharge = Math.max(0, Math.min(maxCharge, enteredFunds * 10));

      this.liveInfo.players_co_power = internalCharge;
      this.playerInfo.players_co_power = internalCharge;
    ...
    okBtn.addEventListener("click", confirmCharge);
