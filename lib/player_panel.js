...
class PlayerPanel {
  ...
  openChargeEditModal() {
      ...
      <div class="awbwenhancements-charge-modal-content">
        ...
        <div class="awbwenhancements-charge-modal-body">
          ...
          <label style="display: block; margin-bottom: 6px; font-weight: bold; font-size: 12px; color: #bbb;">
            Charge Amount (0 to ${maxCharge.toLocaleString()}):
          </label>
          <input type="number" id="awbw-charge-input-val" class="awbwenhancements-charge-input"
                 value="${currentCharge}" min="0" max="${maxCharge}" step="9000">
          <div id="awbw-charge-stars-hint" style="margin-top: 6px; font-size: 12px; color: #39ff14; font-weight: bold;">
            = ${(currentCharge / 90000).toFixed(2)} Stars
          </div>

          <div style="display: flex; gap: 6px; margin-top: 14px; margin-bottom: 8px;">
            <button type="button" class="awbwenhancements-preset-btn" data-val="0">Clear (0)</button>
            <button type="button" class="awbwenhancements-preset-btn" data-val="${this.liveInfo.players_co_max_power}">Full COP (${(this.liveInfo.players_co_max_power / 90000).toFixed(0)}★)</button>
            <button type="button" class="awbwenhancements-preset-btn" data-val="${maxCharge}">Full SCOP (${(maxCharge / 90000).toFixed(0)}★)</button>
          </div>
        </div>
        <div class="awbwenhancements-charge-modal-footer">
          <button type="button" id="awbw-charge-cancel-btn" class="awbwenhancements-btn awbwenhancements-btn-cancel">Cancel</button>
          <button type="button" id="awbw-charge-ok-btn" class="awbwenhancements-btn awbwenhancements-btn-ok">OK</button>
        </div>
      </div>
    `;
    ...
    const updateHint = () => {
      ...
      let stars = (val / 90000).toFixed(2);
      hintEl.textContent = `= ${stars} Stars (${val.toLocaleString()} / ${maxCharge.toLocaleString()})`;
    };
    ...
    presetBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        let val = parseInt(btn.getAttribute("data-val")) || 0;
        inputEl.value = val;
        updateHint();
      });
    });
    ...
    const confirmCharge = () => {
      ...
      if (isNaN(val)) val = 0;
      val = Math.max(0, Math.min(maxCharge, val));

      this.liveInfo.players_co_power = val;
      this.playerInfo.players_co_power = val;
